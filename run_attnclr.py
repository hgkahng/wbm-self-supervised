...
from models.head import LinearHead, MLPHead, CBAMHead
...
def main():
    ...
    # 1. Configurations
    config = AttnCLRConfig.from_command_line()
    config.save()
    ...
    if config.backbone_type == 'resnet':
        if config.backbone_config.endswith('tiny'):
            conv_output_size = config.input_size // 8
        else:
            conv_output_size = config.input_size // 32
    else:
        raise NotImplementedError

    # 3. Model
    backbone = Backbone(BACKBONE_CONFIGS[config.backbone_config], in_channels=IN_CHANNELS[config.data])
    projector = Projector(backbone.out_channels, config.projector_size, input_size=conv_output_size)
    ...
    if config.resume_from_checkpoint is not None:
        ...
        experiment.load_model_from_checkpoint(config.resume_from_checkpoint)
